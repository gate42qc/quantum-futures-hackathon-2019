...
def from_qiskit_to_base_instructions(qc: QuantumCircuit, node_names: List[str]) -> Dict[str, List[BaseInstruction]]:
    q_reg = QuantumRegister(qc.n_qubits)
    c_reg = ClassicalRegister(sum([len(r) for r in qc.cregs]))
    qubits_per_node = int(len(q_reg)/len(node_names))
    new_qc = QuantumCircuit(q_reg, c_reg)
    new_qc.append(qc, q_reg, c_reg)
    transpiled = transpile(new_qc, basis_gates=['cx', 'x', 'h', 't', 'tdg'])
